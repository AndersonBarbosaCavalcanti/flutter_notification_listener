    ...
    class PromoteServiceConfig {
        ...
        companion object {
            ...
            fun fromString(str: String = "{}"): PromoteServiceConfig {
                ...
                // Verifica se a chave "foreground" existe antes de acessá-la
                if (map.has("foreground")) {
                    cfg.foreground = map.getBoolean("foreground")
                }
                
                // Verifica e atribui outras chaves da mesma maneira
                if (map.has("title")) {
                    cfg.title = map.getString("title")
                }
                
                if (map.has("subTitle")) {
                    cfg.subTitle = map.getString("subTitle")
                }
                
                if (map.has("description")) {
                    cfg.description = map.getString("description")
                }
                
                if (map.has("showWhen")) {
                    cfg.showWhen = map.getBoolean("showWhen")
                }
                return cfg
            }
